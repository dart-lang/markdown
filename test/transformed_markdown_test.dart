...
class OurTestTransfomer extends CodeBlockTransformer {
  ...
  OurTestTransfomer()
      : super(handledCodeBlockTypes: ourBlockTypeList);
}
...
class OurAsyncTestTransfomer extends CodeBlockTransformer {
  ...
  Future<String> fakeAsyncTransformer(String codeBlockType, String rawCodeBlock) {
    switch(codeBlockType) {
      case 'testtype':
        return Future.delayed(Duration(milliseconds: 1000), () => 'testtype${rawCodeBlock}testtype' );
      case 'mermaid':
        return Future.value(
...
void main() async {
  ...
  testFile(
    ...
    blockSyntaxes: [ourTestTransformingFencedCodeBlock, FencedCodeBlockSyntax()],
  );
  
  // Test Asynchronous transformed fenced code block 
  testFileAsync(
    'extensions/transformed_fenced_code_blocks.unit',
    blockSyntaxes: [ourAsyncTestTransformingFencedCodeBlock, FencedCodeBlockSyntax()],
  );
}
